...
tag:
  - 无
filename: changyonggainian
slug: qdim0tdwqlb04eqy
docsId: '131988689'
---
...
是一种“临时存储”的类型，Pod存在，emptyDir就会存在，容器挂掉不会引起emptyDir目录下的数据丢失，但是pod被删除或者迁移，emptyDir也会被删除

```shell
spec:
    volumes:
      - name: vol-emptydir # 存储卷的名字，必须
        emptyDir: # emptyDir类型，必须
          sizeLimit: 500Mi # 存储卷的大小，非必须
    containers:
      volumeMounts:
...
emptyDir可用于在一个pod中实现多个容器之间的数据共享。

### hostPath

hostPath允许挂载Node上的文件系统到Pod里面去，即时pod被删除，数据也能持久保存在节点上

```shell
spec:
    volumes:
      - name: vol-hostpath # 存储卷的名字，必须
        hostPath: # hostPath类型，必须
          path: /data # 存储卷的挂载路径，必须
          type: DirectoryOrCreate # 挂载类型，非必须
    containers:
      volumeMounts:
        - name: vol-hostpath # 引用的存储卷名字
