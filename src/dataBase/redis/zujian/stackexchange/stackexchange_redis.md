...
tag:
  - 无
filename: stackexchange_redis
slug: gdahde
docsId: '29714547'
---
连接字符串
```csharp
localhost:6379,defaultDatabase=1,password=123456
```
> 引用的库是StackExchange.Redis

```csharp
string aaa = ConfigurationManager.ConnectionStrings["RedisExchangeHosts"].ConnectionString;
RedisHelper.SetCon(aaa);
//redis.StringSet("aaaa", "123456");

var b = RedisHelper.Get("aaaa");
